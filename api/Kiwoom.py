...
class Kiwoom(QAxWidget):
    ...
    def get_now_price_tal(self, code):
        self.dynamicCall("SetInputValue(QString, QString)", "종목코드", code)
        self.dynamicCall("CommRqData(QString, QString, int, QString)", "opt10001_req", "opt10001", 0, "0003")
    ...
    def _on_receive_tr_data(self, screen_no, rqname, trcode, record_name, next, unused1, unused2, unused3, unused4):
        ...
        if rqname == "opt10081_req":
            ...
            price = int(price.strip().lstrip('+').lstrip('-'))
            self.tr_data = price
    ...
    def get_deposit_tal(self, account_number_selected):
        self.dynamicCall("SetInputValue(QString, QString)", "계좌번호", account_number_selected)
        self.dynamicCall("SetInputValue(QString, QString)", "비밀번호입력매체구분", "00")
        self.dynamicCall("SetInputValue(QString, QString)", "조회구분", "2")
        self.dynamicCall("CommRqData(QString, QString, int, QString)", "opw00001_req", "opw00001", 0, "0002")
    ...
    def send_order_tal(self, rqname, screen_no, order_type, code, order_quantity, order_price, order_classification, account_number_selected, origin_order_number=""):
        order_result = self.dynamicCall("SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",[rqname, screen_no, account_number_selected, order_type, code, order_quantity, order_price,order_classification, origin_order_number])
        return order_result
    ...
    def get_order_tal(self, account_number_selected):
        self.dynamicCall("SetInputValue(QString, QString)", "계좌번호", account_number_selected)
        self.dynamicCall("SetInputValue(QString, QString)", "전체종목구분", "0")
        self.dynamicCall("SetInputValue(QString, QString)", "체결구분", "0")  # 0:전체, 1:미체결, 2:체결
        self.dynamicCall("SetInputValue(QString, QString)", "매매구분", "0")  # 0:전체, 1:매도, 2:매수
    ...
    def get_balance_tal(self, account_number_selected):
        self.dynamicCall("SetInputValue(QString, QString)", "계좌번호", account_number_selected)
        self.dynamicCall("SetInputValue(QString, QString)", "비밀번호입력매체구분", "00")
        self.dynamicCall("SetInputValue(QString, QString)", "조회구분", "1")
        self.dynamicCall("CommRqData(QString, QString, int, QString)", "opw00018_req", "opw00018", 0, "0002")
